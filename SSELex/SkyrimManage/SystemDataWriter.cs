...
namespace SSELex.UIManage
{
    ...
    public class SystemDataWriter
    {
        ...
        public static void SetData(int GetHashKey, string TransData)
        {
            ...
            {
                Translator.TransData[GetHashKey] = NewStr;
            }
            else
            {
                Translator.TransData[GetHashKey] = string.Empty;
            }
        }

        public static string GetTransData(string EditorID, string SetType)
        {
            int GetHashId = SkyrimDataLoader.GenUniqueKey(EditorID, SetType).GetHashCode();
